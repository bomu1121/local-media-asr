...
import {
  NCard, NButton, NSpace, NText, NIcon, NSwitch, NSelect, NSlider, NDivider, NTag, NInput, useMessage,
} from "naive-ui";
import {
  FlashOutline, GitBranchOutline, VolumeHighOutline, TextOutline,
...
const engineOptions = [
  ...
  { label: "精准引擎 (Paraformer-Large)", value: "precise",
    ...
    <NSpace vertical :size="16">
      ...
      <NCard size="small" title="ASR引擎">
        ...
        <NSpace vertical :size="12">
          <NSelect v-model:value="store.settings.engine" :options="engineOptions" placeholder="选择引擎" />
        ...
        <NSpace vertical :size="12">
          ...
          <div class="setting-row" v-if="store.settings.vad.enabled">
            ...
            <div class="setting-control">
              <NSlider v-model:value="store.settings.vad.threshold" :min="0.1" :max="0.9" :step="0.05" style="width: 200px;" />
              <NText depth="3" style="font-size: 12px; width: 40px;">{{ store.settings.vad.threshold.toFixed(2) }}</NText>
            </div>
          </div>
          <div class="setting-row" v-if="store.settings.vad.enabled">
            <NText>最小语音段 (秒)</NText>
            <NSlider v-model:value="store.settings.vad.minSpeechDuration" :min="0.1" :max="2.0" :step="0.1" style="width: 200px;" />
          </div>
          <div class="setting-row" v-if="store.settings.vad.enabled">
            <NText>最大段时长 (秒)</NText>
            <NSlider v-model:value="store.settings.vad.maxSegmentDuration" :min="15" :max="120" :step="5" style="width: 200px;" />
          </div>
        </NSpace>
      </NCard>
      ...
      <NCard size="small" title="导出设置">
        <NSpace vertical :size="12">
          <div class="setting-row">
            <NText>默认导出格式</NText>
            <NSelect v-model:value="store.settings.exportFormat" :options="exportOptions" style="width: 160px;" size="small" />
          </div>
          <div class="setting-row">
            <NText>导出目录</NText>
